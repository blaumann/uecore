...
struct MANGOS_DLL_DECL boss_lucifronAI : public ScriptedAI
{
    ...
    void Reset()
    {
		if(m_pInstance)
		{
			m_pInstance->SetData(DATA_LUCIFRON_PROGRESS, NOT_STARTED);
	...
	{
		if(m_pInstance)
			m_pInstance->SetData(DATA_LUCIFRON_PROGRESS, IN_PROGRESS);
    }

	void JustDied(Unit* pKiller)
    {
		if(m_pInstance)
			m_pInstance->SetData(DATA_LUCIFRON_PROGRESS, DONE);
    }
