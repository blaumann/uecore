...
void hyjalAI::Retreat()
{
    ...
    MaNGOS::CreatureListSearcher<MaNGOS::AllFriendlyCreaturesInGrid> creature_searcher(m_creature, creatures, creature_check);
...
void hyjalAI::SpawnVeins()
{
    if(Faction == 0)
    {
        ...
        for (uint8 i = 0; i<7; i++)
        {
            //GameObject* gem = m_creature->GetGameObject(ANCIENT_VEIN,VeinPos[i][0],VeinPos[i][1],VeinPos[i][2],VeinPos[i][3],VeinPos[i][4],VeinPos[i][5],VeinPos[i][6],VeinPos[i][7]);
            //if(gem)
...
void hyjalAI::DeSpawnVeins()
{
   /* if(!pInstance)return;    
    if(Faction == 1)
    {
        Creature* pUnit = (Creature*)Unit::GetUnit(0 ,pInstance->GetData64(DATA_JAINAPROUDMOORE));
        if (!pUnit)return;
    ...
    {
        Creature* pUnit = (Creature*)Unit::GetUnit(0 ,pInstance->GetData64(DATA_THRALL));
        if (!pUnit)return;
        hyjalAI* ai = ((hyjalAI*)pUnit->AI());
        if(!ai)return;
        for (uint8 i = 7; i<14; i++)
            ...
            case JAINA: 
                if(pInstance->GetData(DATA_ALLIANCE_RETREAT))
                {
                    ...
                    //for(uint8 i = 0; i < 92; i++)//summon fires
                        //m_creature->GetGameObject(FLAMEOBJECT,AllianceFirePos[i][0],AllianceFirePos[i][1],AllianceFirePos[i][2]);
...
void hyjalAI::RespawnNearPos(float x, float y)
{
    ...
    MaNGOS::WorldObjectWorker<MaNGOS::RespawnDo> worker(m_creature, u_do);
    ...
    switch(faction)
    {
        case 0://alliance
            ...
            for(uint8 i = 0; i < 5; i++)//summon 5 gargoyles
            {
                ...
                if(pUnit)
                {
                    pUnit->SetCombatStartPosition(AllianceOverrunGargPos[i][0], AllianceOverrunGargPos[i][1], AllianceOverrunGargPos[i][2]);
                    ((hyjal_trashAI*)pUnit->AI())->faction = Faction;                    
                    ((hyjal_trashAI*)pUnit->AI())->IsOverrun = true;
                    ((hyjal_trashAI*)pUnit->AI())->OverrunType = i;
                }
        ...
        //case 1://horde    
            ...
            for(uint8 i = 0; i < 26; i++)//summon infernals
            {
                ...
                if(pUnit)
                {
                    pUnit->SetCombatStartPosition(InfernalSPWP[i][0], InfernalSPWP[i][1], InfernalSPWP[i][2]);
                    ((hyjal_trashAI*)pUnit->AI())->faction = Faction;                    
                    ((hyjal_trashAI*)pUnit->AI())->IsOverrun = true;
                    ((hyjal_trashAI*)pUnit->AI())->OverrunType = i;
                }            
