...
{
	boss_shirrak_the_dead_watcherAI(Creature* pCreature) : ScriptedAI(pCreature) {Reset();} 
	...
	void UpdateAI(const uint32 diff)
	{
		...
		if(Focus_Fire_Timer < diff)
		{
			if(Intern_Focus_Fire_Timer < 400 && Intern_Focus_Fire_Counter == 0)
			{
				...
				}else Intern_Focus_Fire_Counter = 4;
			}
...
{
	mob_focusfireAI(Creature* pCreature) : Scripted_NoMovementAI(pCreature)
	{
		Reset();
	}
...
CreatureAI* GetAI_boss_shirrak_the_dead_watcherAI(Creature* pCreature)
{
...
CreatureAI* GetAI_mob_focusfireAI(Creature* pCreature)
{
	return new mob_focusfireAI(pCreature);
}
