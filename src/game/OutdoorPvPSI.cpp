...
bool OutdoorPvPSI::HandleAreaTrigger(Player *plr, uint32 trigger)
{
    ...
    case SI_AREATRIGGER_A:
        if(plr->GetTeam() == ALLIANCE && plr->HasAura(SI_SILITHYST_FLAG))
        {
            ...
            {
                BuffTeam(ALLIANCE);
                sWorld.SendZoneText(OutdoorPvPSIBuffZones[0],objmgr.GetMangosStringForDBCLocale(LANG_OPVP_SI_CAPTURE_A));
    ...
    case SI_AREATRIGGER_H:
        if(plr->GetTeam() == HORDE && plr->HasAura(SI_SILITHYST_FLAG))
        {
            ...
            {
                BuffTeam(HORDE);
                sWorld.SendZoneText(OutdoorPvPSIBuffZones[0],objmgr.GetMangosStringForDBCLocale(LANG_OPVP_SI_CAPTURE_H));
