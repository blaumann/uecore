...
bool AchievementCriteriaData::IsValid(AchievementCriteriaEntry const* criteria)
{
    ...
    switch(dataType)
    {
        ...
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_LEVEL:
            if(raw.value1<0 || raw.value1>STRONG_MAX_LEVEL)
            {
                sLog.outErrorDb( "Table `achievement_criteria_data` (Entry: %u Type: %u) for data type ACHIEVEMENT_CRITERIA_DATA_TYPE_T_LEVEL (%u) have wrong minlevel in value1 (%u), ignore.",
                    criteria->ID, criteria->requiredType,dataType,raw.value1);
                return false;
            }
            return true;
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_GENDER:
            if(raw.value1!=GENDER_MALE && raw.value1!=GENDER_FEMALE)
            {
...
bool AchievementCriteriaData::Meets(Player const* source, Unit const* target, uint32 miscvalue1 /*= 0*/) const
{
    switch(dataType)
    {
        ...
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_LEVEL:
            if (!target || target->GetTypeId()!=TYPEID_PLAYER)
                return false;
            if(((Player*)target)->getLevel() < raw.value1)
                return false;
            return true;
        case ACHIEVEMENT_CRITERIA_DATA_TYPE_T_GENDER:
            if (!target || target->GetTypeId()!=TYPEID_PLAYER)
                return false;
            if(((Player*)target)->getGender() != raw.value1)
                return false;
            return true;
    }
    return false;
