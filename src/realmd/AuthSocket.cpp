...
bool AuthSocket::_HandleLogonProof()
{
    ...
    if (!memcmp(M.AsByteArray(), lp.M1, 20))
    {
        ...
        dbRealmServer.DirectPExecute("UPDATE account SET sessionkey = '%s', last_ip = '%s', last_login = NOW(), locale = '%u', failed_logins = 0 WHERE username = '%s'", K_hex, GetRemoteAddress().c_str(), GetLocaleByName(_localizationName), _safelogin.c_str() );
